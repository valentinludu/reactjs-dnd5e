...
const CharacterConfig = () => {
    ...
    const { data: classes, isLoading: isClassesLoading, } = useQuery(["classesList","classes"], fetchData, { staleTime: 10000 });
    const classesList = classes && classes.results.map(({index, name}) => ({label: name, value: index}));
    const { data: racesData, loading: isRacesLoading } = useQuery(["racesList", "races"], fetchData, { staleTime: 10000 });
    const races = ((racesData && racesData.results) || []).map(({index, name}) => ({label: name, value: index}));

    const initialValues = { name: "", age: "", classList: { label: "", value: "" }, race: { label: "", value: "" } };
            ...
            >
                {({ isSubmitting, errors }) => (
                        <Form>
                            ...
                            <FieldInput type="number" name="age" error={errors.age} disabled={isSubmitting} />
                            <FieldError error={errors.age} />
                            <FieldLabel htmlFor="race">Race</FieldLabel>
                            <Field name="race" component={SelectField} options={races} disabled={isSubmitting} error={errors.race && errors.race.value} />
                            <FieldError error={errors.race && errors.race.value} />
                            <FieldLabel htmlFor="class">Class</FieldLabel>
                            <Field name="class" component={SelectField} options={classesList} disabled={isSubmitting} error={errors.class && errors.class.value} />
